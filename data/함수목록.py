...
* EncodeSwitchState(s)
* EncodeUnit(u)
    이름을 트리거에서 쓰는 번호로 바꿉니다. ** 인덱스랑 차이가 있을 수 있습니다 ** :
 
        EncodeLocation('Location 0')  # 1
...
* unProxy(x)  ** 고급 **
    x에 여러가지 타입을 적용할 수 있는데, unProxy는 x에서 이러한 타입을 모두 뗴는
    연산이니다. 아래와 같이 두 변수를 비교할 때 씁니다. ::
...
* f_epdread_epd_safe(targetplayer)
    위 함수들의 safe 버젼입니다. safe 버젼은 targetplayer에 해당하는 메모리를
    전혀 손대지 않고 값을 읽습니다. Write 권한이 없는 메모리를 읽을 때 유용하게
    사용할 수 있습니다. _safe가 없는 버젼보다 2배정도 느립니다.
...
[출처] eudplib 함수 목록  (스타 에디터 아카데미 - 맵 제작 연구·커뮤니티) |작성자 트리거왕