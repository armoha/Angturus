...
def f_process_args_for_ct(*args):
    ct_args = tuple(ct.f_1c(x) if isUnproxyInstance(x, str) else x for x in args)
    return ct_args
...
def f_setTxtPtr(line):
    ...
    if EUDIf()(prev % 2 == 0):
        if EUDIf()(
            EUDSCAnd()(MemoryEPD(dst, Exactly, odds_1))(
        ...
        EUDEndIf()
    if EUDElseIf()(
        EUDSCAnd()(MemoryEPD(dst, AtMost, even_1 + 0xFFFF))(
...
def f_effectBase(*args, line=0, autoreset=True, timer=None):
    ...
    w1, w2, w3 = [b2i2(u2b(x)) for x in (c0 + c1, c2 + c3, c4 + c5)]
    SetVariables(
        [odds_1, odds_2, even_1, even_2, display_text, restore_txtPtr],
        [
            w1 + w2 * 0x10000,
            w3 + 0x0D0D * 0x10000,
            w1 * 0x10000,
    ...
    if timer is None:
        ...
        if re.search(r"<\S* object at 0x[0-9A-F]+>", key) is not None:
            raise Exception(
                "object가 인자로 쓰이면 timer를 지정해야합니다; Must assign timer when arguments have object: {}".format(
                    args
                )
            )
    else:
...
def f_fadein(
    *args, color=None, interval=1, line=0, autoreset=True, timer=None, txtlen=None
):
    if txtlen is None:
    ...
    RawTrigger(
        ...
        actions=[_interval.SetNumber(0), _timer.AddNumber(1)],
    )
...
def _warning():
    for key in timers:
        if key not in effect_keys:
            raise Exception("미사용 타이머를 reset했습니다; reset unused timer: {}".format(key))
        if key not in reset_keys and key in alert_manual_reset:
            raise Exception("수동 reset 필요; need manual reset for timer: {}".format(key))
